...
class OccupancyGrid
{
    ...
    void resize_image(size_t w, size_t h)
    {
        size_t offset_w = max(0.0f, ceil(w/occupancy_grid.info.resolution - image.cols) / 2);
        size_t offset_h = max(0.0f, ceil(h/occupancy_grid.info.resolution - image.rows) / 2);
        cv::copyMakeBorder(image, image, offset_h, offset_h, offset_w, offset_w,cv::BORDER_CONSTANT, 0);
        cv::copyMakeBorder(free, free, offset_h, offset_h, offset_w, offset_w,cv::BORDER_CONSTANT, 0);
        cv::copyMakeBorder(occupied, occupied, offset_h, offset_h, offset_w, offset_w,cv::BORDER_CONSTANT, 0);
        cv::copyMakeBorder(prob, prob, offset_h, offset_h, offset_w, offset_w,cv::BORDER_CONSTANT, 0);
    ...
    void process_free_and_occupied()
    {
        for(int i=0; i!=image.rows;i++)
        {
            for(int j=0; j!=image.cols;j++)
            {
                ...
                if(prob_free>0.55)image.at<int8_t>(i,j) = 0;
